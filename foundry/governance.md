...
- **Si la promoción sube el canal a `stable`**: mover físicamente la carpeta de `skills/.experimental/<nombre>/` a `skills/<nombre>/` en el mismo commit de la ronda, y rehacer cualquier symlink que apunte a la ruta vieja (ej. `~/.claude/skills/<nombre>`) — un symlink apunta a una ruta fija; si la carpeta se mueve y el symlink no se rehace, queda roto en silencio hasta que alguien intente usar la skill y falle. Verificar con `ls -la ~/.claude/skills/<nombre>` que el symlink resuelve después del cambio, no solo que el `mv` corrió sin error.
