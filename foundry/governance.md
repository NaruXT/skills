...
- **Cualquier ronda que mueva físicamente la carpeta de una skill** (promoción a `stable`, pero también una deprecación que la archiva en `skills/deprecated/`, o cualquier reorganización futura): mover la carpeta en el mismo commit de la ronda, y rehacer cualquier symlink que apunte a la ruta vieja (ej. `~/.claude/skills/<nombre>`) — un symlink apunta a una ruta fija; si la carpeta se mueve y el symlink no se rehace, queda roto en silencio hasta que alguien intente usar la skill y falle. Verificar con `ls -la ~/.claude/skills/<nombre>` que el symlink resuelve después del cambio, no solo que el `mv` corrió sin error. `scripts/validate-skills.mjs` detecta la mitad de este problema (si `maturity.json` quedó inconsistente con la ubicación física), pero no detecta un symlink roto — esa verificación sigue siendo manual.
